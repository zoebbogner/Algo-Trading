...
from ..utils.config import config_manager, settings
...
@click.option(
    "--mode",
    default="SIM",
    type=click.Choice(["SIM", "PAPER", "LIVE"]),
    help="Trading mode",
)
@click.option(
    "--symbols", default="BTC/USD,ETH/USD", help="Trading symbols (comma-separated)"
)
...
def validate_config(config_file: Optional[str]):
    ...
    if config_file:
        ...
        try:
            ...
            console.print(
                "[green]All configuration files loaded successfully![/green]"
            )
