...
class AppSettings(BaseSettings):
    ...
    broker_sandbox_url: Optional[str] = Field(
        default=None, description="Broker sandbox URL"
    )

    # Data configuration
    data_cache_dir: str = Field(
        default="Algo-Trading/data/cache", description="Data cache directory"
    )
    data_raw_dir: str = Field(
        default="Algo-Trading/data/raw", description="Raw data directory"
    )
    data_processed_dir: str = Field(
        default="Algo-Trading/data/processed", description="Processed data directory"
    )
    data_features_dir: str = Field(
        default="Algo-Trading/data/features", description="Features directory"
    )
    ...
    drawdown_cooldown_minutes: int = Field(
        default=60, description="Drawdown cooldown minutes"
    )
    per_trade_risk: float = Field(default=0.005, description="Per trade risk")

    # Trading configuration
    default_symbols: str = Field(
        default="BTC/USD,ETH/USD", description="Default trading symbols"
    )
    bar_interval: str = Field(default="1m", description="Bar interval")
    market_order_timeout_seconds: int = Field(
        default=30, description="Market order timeout"
    )
    limit_order_timeout_seconds: int = Field(
        default=300, description="Limit order timeout"
    )

    # State management
    state_dir: str = Field(default="Algo-Trading/state", description="State directory")
    portfolio_dir: str = Field(
        default="Algo-Trading/state/portfolios", description="Portfolio directory"
    )
    checkpoint_dir: str = Field(
        default="Algo-Trading/state/checkpoints", description="Checkpoint directory"
    )

    # Reports
    reports_dir: str = Field(
        default="Algo-Trading/reports", description="Reports directory"
    )
    daily_reports_dir: str = Field(
        default="Algo-Trading/reports/daily", description="Daily reports directory"
    )
    run_reports_dir: str = Field(
        default="Algo-Trading/reports/runs", description="Run reports directory"
    )
