...
try:
    ...
    from src.data_models.market import Bar
...
class CryptocurrencyCorrelationAnalyzer:
    ...
    def __init__(self, config: dict):
        ...
        self.regime_thresholds = config.get(
            "regime_thresholds",
            {
        ...
        print("🔗 Cryptocurrency Correlation Analyzer initialized")
    ...
    async def initialize(self, data_manager: EnhancedDataManager):
        ...
        print("✅ Correlation analyzer initialized with data manager")
    ...
    async def analyze_all_correlations(
        ...
        for symbol in symbols:
            try:
                ...
                if data and len(data) >= self.min_correlation_samples:
                    ...
                    print(
                        f"⚠️ Insufficient data for {symbol}: {len(data) if data else 0} bars"
                    )
        ...
        print(
            f"📊 Calculating correlation matrix for {len(symbols_with_data)} symbols..."
        )
        ...
        self.correlation_matrix = pd.DataFrame(
            correlation_matrix, index=symbols_with_data, columns=symbols_with_data
        )
        ...
        self.correlation_history[
            datetime.now(UTC)
        ] = self.correlation_matrix.copy()

        print(f"✅ Correlation analysis completed for {len(correlations)} pairs")
        return correlations

    async def analyze_bitcoin_impact(
        self, target_symbols: list[str] = None
    ) -> dict[str, BitcoinImpactAnalysis]:
        """
        ...
        if target_symbols is None:
            target_symbols = [
                s for s in self.data_manager.symbols if s != self.bitcoin_symbol
            ][:30]

        print(
            f"📈 Analyzing Bitcoin impact on {len(target_symbols)} cryptocurrencies..."
        )
        ...
        for symbol in target_symbols:
            try:
                ...
                if not np.isnan(correlation):
                    ...
                    print(
                        f"   ✅ {symbol}: Correlation={correlation:.3f}, Beta={beta:.3f}"
                    )
                else:
    ...
    def _calculate_correlation(
        self, data1: list[Bar], data2: list[Bar]
    ) -> Optional[float]:
    ...
    def _calculate_lag_correlations(
        self, bitcoin_returns: list[float], target_returns: list[float]
    ) -> dict[int, float]:
        ...
        for lag in range(1, min(self.max_lag_days + 1, len(bitcoin_returns) // 2)):
            if len(bitcoin_returns) > lag and len(target_returns) > lag:
                ...
                if (
                    len(bitcoin_lead) >= self.min_correlation_samples
                    and len(target_lag) >= self.min_correlation_samples
                ):
    ...
    def _calculate_regime_correlations(
        ...
        if np.sum(bull_mask) >= self.min_correlation_samples:
            bull_correlation = np.corrcoef(
                bitcoin_returns_array[bull_mask], np.array(target_returns)[bull_mask]
            )[0, 1]
        ...
        if np.sum(bear_mask) >= self.min_correlation_samples:
            bear_correlation = np.corrcoef(
                bitcoin_returns_array[bear_mask], np.array(target_returns)[bear_mask]
            )[0, 1]
        ...
        sideways_mask = (
            np.abs(bitcoin_returns_array) <= self.regime_thresholds["sideways"]
        )
    ...
    def _create_correlation_metrics(
        self,
        symbol1: str,
        symbol2: str,
        correlation: float,
        ...
        if min_length >= 10:
            ...
            rolling_corr = np.corrcoef(
                returns1_aligned[-rolling_window:], returns2_aligned[-rolling_window:]
            )[0, 1]
        else:
    ...
    def get_bitcoin_dependent_coins(
        self, threshold: float = None
        ...
        for symbol, impact in self.bitcoin_impact_cache.items():
            if abs(impact.bitcoin_correlation) >= threshold:
                bitcoin_dependent.append(
                    (symbol, impact.bitcoin_correlation, impact.bitcoin_beta)
                )
    ...
    def _get_trading_implication(self, impact: BitcoinImpactAnalysis) -> str:
        ...
        if abs(impact.bitcoin_correlation) > 0.8:
            ...
            if impact.bitcoin_correlation > 0:
                return (
                    "Moderate Bitcoin correlation - Bitcoin movements provide guidance"
                )
            else:
                return "Moderate inverse Bitcoin correlation - consider contrarian positions"
        elif abs(impact.bitcoin_correlation) > 0.4:
            return (
                "Low Bitcoin correlation - trade based on individual coin fundamentals"
            )
        else:
    ...
    def generate_correlation_report(self) -> str:
        ...
        if self.bitcoin_impact_cache:
            ...
            report.append(
                f"High Bitcoin Correlation (≥0.6): {len(bitcoin_dependent)} coins"
            )
            ...
            independent_coins = [
                s
                for s, i in self.bitcoin_impact_cache.items()
                if abs(i.bitcoin_correlation) < 0.3
            ]
            report.append(
                f"Independent of Bitcoin (<0.3): {len(independent_coins)} coins"
            )
            report.append("")
