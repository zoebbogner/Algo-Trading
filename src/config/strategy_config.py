...
class PortfolioOptimizationConfig:
    ...
    available_methods: List[str] = field(default_factory=lambda: [
        'efficient_frontier', 'risk_parity', 'maximum_sharpe', 
        'minimum_variance', 'black_litterman'
    ])
...
class DataConfig:
    ...
    # Symbols
    default_symbols: List[str] = field(default_factory=lambda: ['BTC/USDT', 'ETH/USDT', 'ADA/USDT'])
...
class UltraEnhancedStrategyConfig:
    ...
    version: str = "3.0"
    description: str = "Advanced momentum strategy with ML confirmation and risk management"
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> 'UltraEnhancedStrategyConfig':
        ...
        opt_config = PortfolioOptimizationConfig(**config_dict.get('optimization_config', {}))
        ...
        return cls(
            ...
            use_portfolio_optimization=config_dict.get('use_portfolio_optimization', True),
            ml=ml_config,
...
def validate_config(config: UltraEnhancedStrategyConfig) -> List[str]:
    ...
    if config.risk_management.position_size < config.risk_management.min_position_size:
        issues.append(f"Position size {config.risk_management.position_size} below minimum {config.risk_management.min_position_size}")
    
    if config.risk_management.position_size > config.risk_management.max_position_size:
        issues.append(f"Position size {config.risk_management.position_size} above maximum {config.risk_management.max_position_size}")
    
    # Validate entry thresholds
    if config.entry_exit.min_entry_threshold < 0 or config.entry_exit.max_entry_threshold > 100:
    ...
    if config.advanced_risk.var_confidence_level < 0.5 or config.advanced_risk.var_confidence_level > 0.999:
