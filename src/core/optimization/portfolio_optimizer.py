...
class PortfolioOptimizer:
    ...
    def __init__(self, config: dict):
        ...
        self.rebalancing_threshold = config.get(
    ...
    def optimize_portfolio(
        ...
        result["optimization_timestamp"] = datetime.now(UTC)
    ...
    def _calculate_expected_returns(self, returns_matrix: np.ndarray) -> np.ndarray:
        ...
        shrunk_mean = (
            shrinkage_factor * global_mean + (1 - shrinkage_factor) * simple_mean
        )
    ...
    def _calculate_covariance_matrix(self, returns_matrix: np.ndarray) -> np.ndarray:
        ...
        shrinkage_factor = 0.3
        shrunk_cov = (
            shrinkage_factor * target_matrix + (1 - shrinkage_factor) * sample_cov
        )
    ...
    def _optimize_efficient_frontier(
        ...
        target_returns = np.linspace(
            np.min(expected_returns), np.max(expected_returns), 50
        )
            ...
            try:
                ...
                if result.success:
                    ...
                    portfolio_volatility = np.sqrt(
                        self._portfolio_variance(weights, covariance_matrix)
                    )
                    sharpe_ratio = (
                        portfolio_return - constraints["risk_free_rate"] / 252
    ...
    def _optimize_risk_parity(
        self, covariance_matrix: np.ndarray, constraints: dict
    ) -> dict:
        ...
        def risk_parity_objective(weights):
            ...
            risk_parity_penalty = sum(
                (rc - target_risk) ** 2 for rc in risk_contributions
            )
        ...
        try:
            ...
            if result.success:
                ...
                for i in range(n_assets):
                    marginal_risk = (
                        np.dot(covariance_matrix[i, :], weights) / portfolio_vol
                    )
                    risk_contribution = weights[i] * marginal_risk
                    risk_contributions.append(risk_contribution)
                ...
                return {
                    "error": "Risk parity optimization failed",
                    "optimization_success": False,
                }

        except Exception as e:
            return {
                "error": f"Risk parity optimization error: {e}",
                "optimization_success": False,
            }
    ...
    def _optimize_maximum_sharpe(
        ...
        try:
            ...
            if result.success:
                ...
                return {
                    "error": "Maximum Sharpe optimization failed",
                    "optimization_success": False,
                }

        except Exception as e:
            return {
                "error": f"Maximum Sharpe optimization error: {e}",
                "optimization_success": False,
            }
    ...
    def _optimize_minimum_variance(
        ...
        try:
            ...
            if result.success:
                ...
                return {
                    "error": "Minimum variance optimization failed",
                    "optimization_success": False,
                }

        except Exception as e:
            return {
                "error": f"Minimum variance optimization error: {e}",
                "optimization_success": False,
            }
    ...
    def calculate_portfolio_metrics(
        ...
        for i in range(len(weights)):
            marginal_risk = (
                np.dot(covariance_matrix[i, :], weights) / portfolio_volatility
            )
            risk_contribution = weights[i] * marginal_risk
            risk_contributions.append(risk_contribution)
        ...
        return {
            "expected_return": portfolio_return,
            "volatility": portfolio_volatility,
            "sharpe_ratio": (portfolio_return - self.risk_free_rate / 252)
            / portfolio_volatility
    ...
    def rebalance_portfolio(
        ...
        for position in current_portfolio.positions:
            current_weights[position.symbol] = (
        ...
        for symbol in set(current_weights.keys()) | set(target_weights.keys()):
            ...
            if (
                abs(target_value - current_value) / total_value
