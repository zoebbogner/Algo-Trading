...
class UltraEnhancedMomentumStrategy(Strategy):
    ...
    def optimize_portfolio_allocation(self, current_prices: Dict[str, float]) -> Dict:
        ...
        methods = ['efficient_frontier', 'risk_parity', 'maximum_sharpe', 'minimum_variance']
        ...
        if best_result:
            print(f"✅ Best optimization: {best_result.get('method', 'unknown')} - Sharpe: {best_sharpe:.3f}")
    ...
    def _should_enter_with_heat_map(self, symbol: str, portfolio: Portfolio, heat_map: Dict[str, Any]) -> bool:
        ...
        if heat_map['position_concentration'] > 0.4:  # 40% max concentration
            print(f"⚠️ ULTRA-ENHANCED STRATEGY: {symbol} Portfolio too concentrated ({heat_map['position_concentration']:.1%})")
            return False
        
        # Don't enter if total exposure is too high
        if heat_map['total_exposure'] > 0.8:  # 80% max total exposure
            print(f"⚠️ ULTRA-ENHANCED STRATEGY: {symbol} Total exposure too high ({heat_map['total_exposure']:.1%})")
            return False
        
        # Don't enter if correlation risk is too high
        if heat_map['correlation_risk']:
            print(f"⚠️ ULTRA-ENHANCED STRATEGY: {symbol} Correlation risk too high (crypto exposure: {heat_map['crypto_exposure']:.1%})")
            return False
    ...
    def generate_signals(self, market_data: MarketData, portfolio: Portfolio) -> Dict[str, Any]:
        ...
        if self.risk_modeling:
            ...
            if risk_metrics:
                print(f"📊 Advanced Risk Metrics: Portfolio VaR: {risk_metrics.get('portfolio_var', {}).get('portfolio_var_percentage', 0):.2%}")
        ...
        if self.portfolio_optimizer:
            ...
            if portfolio_optimization:
                print(f"🎯 Portfolio Optimization: {portfolio_optimization.get('method', 'unknown')} completed")
        
        for symbol, bars in market_data.bars.items():
            if not bars or len(bars) < self.trend_ma_period:
                print(f"❌ ULTRA-ENHANCED STRATEGY: {symbol} insufficient data ({len(bars)} bars)")
                continue
            ...
            if not current_position:
                print(f"🚀 ULTRA-ENHANCED STRATEGY: {symbol} has no position, checking entry conditions...")
                ...
                print(f"📊 ULTRA-ENHANCED STRATEGY: {symbol} Portfolio heat map - Total exposure: {heat_map['total_exposure']:.1%}, Crypto: {heat_map['crypto_exposure']:.1%}")
                
                # Check correlation risk
                if self._check_correlation_risk(symbol, portfolio):
                    print(f"⚠️ ULTRA-ENHANCED STRATEGY: {symbol} correlation risk too high, skipping")
                    continue
                
                # Calculate adaptive entry threshold
                adaptive_threshold = self._calculate_adaptive_entry_threshold(indicators)
                print(f"🎯 ULTRA-ENHANCED STRATEGY: {symbol} Adaptive entry threshold: {adaptive_threshold}/100")
                ...
                if (indicators['short_ma'] > indicators['medium_ma'] > indicators['long_ma']):
                    ...
                    print(f"✅ ULTRA-ENHANCED STRATEGY: {symbol} Multi-timeframe uptrend confirmed")
                
                # 2. RSI momentum (20 points)
                if 30 < indicators['rsi'] < 70:
                    entry_score += 20
                    entry_reasons.append("RSI in healthy range")
                    print(f"✅ ULTRA-ENHANCED STRATEGY: {symbol} RSI healthy: {indicators['rsi']:.1f}")
                
                # 3. Volume confirmation (15 points)
                if indicators['volume_ratio'] > 1.0:
                    entry_score += 15
                    entry_reasons.append("Above average volume")
                    print(f"✅ ULTRA-ENHANCED STRATEGY: {symbol} Volume confirmed: {indicators['volume_ratio']:.2f}")
                
                # 4. Trend strength (15 points)
                if indicators['adx'] > 20:
                    entry_score += 15
                    entry_reasons.append("Strong trend")
                    print(f"✅ ULTRA-ENHANCED STRATEGY: {symbol} Trend strength: {indicators['adx']:.1f}")
                
                # 5. Volatility opportunity (10 points)
                if indicators['volatility'] > self.volatility_threshold:
                    entry_score += 10
                    entry_reasons.append("Good volatility")
                    print(f"✅ ULTRA-ENHANCED STRATEGY: {symbol} Volatility: {indicators['volatility']:.3f}")
                ...
                if self.ml_confirmation and self.ml_confirmation.models:
                    ml_confirmed, ml_confidence, ml_details = self.ml_confirmation.confirm_signal(bars, indicators)
                    
                    if ml_details.get('ml_available', False):
                        print(f"🤖 ML Confirmation: {ml_confirmed}, Confidence: {ml_confidence:.3f}")
                        
                        if ml_confirmed:
                            entry_score += 10  # Bonus points for ML confirmation
                            entry_reasons.append("ML confirmed")
                            print(f"✅ ULTRA-ENHANCED STRATEGY: {symbol} ML confirmation bonus: +10 points")
                        else:
                            entry_score -= 20  # Penalty for ML rejection
                            print(f"❌ ULTRA-ENHANCED STRATEGY: {symbol} ML rejection penalty: -20 points")
                ...
                if entry_score >= adaptive_threshold and ml_confirmed:
                    ...
                    print(f"🚀 ULTRA-ENHANCED STRATEGY: {symbol} BUY SIGNAL! Score: {entry_score}/{adaptive_threshold}")
                    print(f"   Position Size: {position_size:.1%}, Quantity: {entry_signals[symbol]['quantity']:.4f}")
                    print(f"   ML Confirmed: {ml_confirmed}, Confidence: {ml_confidence:.3f}")
                else:
                    print(f"❌ ULTRA-ENHANCED STRATEGY: {symbol} Entry score too low: {entry_score}/{adaptive_threshold}")
                    if not ml_confirmed:
                        print(f"   ML Rejection: Confidence {ml_confidence:.3f} below threshold")
            
            # Exit signals for existing positions
            elif current_position:
                print(f"📉 ULTRA-ENHANCED STRATEGY: {symbol} has position, checking exit conditions...")
                ...
                if pnl_pct <= -self.stop_loss_pct:
                    ...
                    print(f"🛑 ULTRA-ENHANCED STRATEGY: {symbol} STOP LOSS triggered: {pnl_pct:.2%}")
                
                # 2. Take profit (better risk-reward)
                elif pnl_pct >= self.take_profit_pct:
                    should_exit = True
                    exit_reason = f"Take profit: {pnl_pct:.2%}"
                    print(f"🎯 ULTRA-ENHANCED STRATEGY: {symbol} TAKE PROFIT triggered: {pnl_pct:.2%}")
                ...
                # 3. Trailing stop (protect profits)
                    ...
                    if current_price <= trailing_stop_price:
                        ...
                        print(f"📉 ULTRA-ENHANCED STRATEGY: {symbol} TRAILING STOP triggered: {pnl_pct:.2%}")
                ...
                # 4. Technical exit signals
                    ...
                    print(f"📊 ULTRA-ENHANCED STRATEGY: {symbol} TECHNICAL EXIT: MA crossover + RSI overbought")
                ...
                # 5. Market regime change
                    ...
                    print(f"🌊 ULTRA-ENHANCED STRATEGY: {symbol} MARKET REGIME EXIT: Strong downtrend detected")
                
                # 6. Advanced risk management exit
                if self.risk_modeling and risk_metrics:
                    portfolio_var = risk_metrics.get('portfolio_var', {})
                    if portfolio_var:
                        var_percentage = portfolio_var.get('portfolio_var_percentage', 0)
                        if var_percentage > 0.05:  # If portfolio VaR > 5%
                            should_exit = True
                            exit_reason = f"Risk management: Portfolio VaR {var_percentage:.2%} > 5%"
                            print(f"⚠️ ULTRA-ENHANCED STRATEGY: {symbol} RISK MANAGEMENT EXIT: VaR {var_percentage:.2%}")
                ...
                if should_exit:
                    ...
                    print(f"📉 ULTRA-ENHANCED STRATEGY: {symbol} SELL SIGNAL: {exit_reason}")
                else:
                    print(f"⏳ ULTRA-ENHANCED STRATEGY: {symbol} Holding position, no exit signal")
        ...
        result = {
            ...
            'advanced_metrics': {
                ...
                'ml_models_available': self.ml_confirmation is not None and self.ml_confirmation.models,
                'risk_modeling_available': self.risk_modeling is not None,
                'portfolio_optimization_available': self.portfolio_optimizer is not None
            }
        }
