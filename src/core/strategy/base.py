...
class Strategy(ABC):
    ...
    def calculate_position_size(self, symbol: str, market_data: MarketData, portfolio: Portfolio) -> Decimal:
    ...
    def _calculate_performance_metrics(self) -> None:
        ...
        avg_win = sum(t.get("pnl", 0) for t in self.trades if t.get("pnl", 0) > 0) / winning_trades if winning_trades > 0 else 0
        avg_loss = sum(t.get("pnl", 0) for t in self.trades if t.get("pnl", 0) < 0) / losing_trades if losing_trades > 0 else 0
