...
class Feature(TimeSeriesEntity):
    ...
    lookback_period: Optional[int] = Field(
        None, description="Lookback period for calculation"
    )
    source: str = Field(
        ..., description="Source of the feature (e.g., 'technical', 'price', 'volume')"
    )
...
class MarketData(TimeSeriesEntity):
    ...
    symbol: Optional[str] = Field(
        None, description="Primary symbol (optional for multi-symbol data)"
    )
    bars: dict[str, list[Bar]] = Field(
        default_factory=dict, description="OHLCV bar data by symbol"
    )
    features: dict[str, list[Feature]] = Field(
        default_factory=dict, description="Engineered features by symbol"
    )
    ...
    timestamp_received: Optional[datetime] = Field(
        None, description="When data was received"
    )
