...
class Portfolio(BaseEntity):
    ...
    exposure_gross: Decimal = Field(
        ..., description="Gross exposure (sum of absolute position values)"
    )
    exposure_net: Decimal = Field(
        ..., description="Net exposure (sum of position values)"
    )
