...
class MLSignalConfirmation:
    ...
    def __init__(self, config: Dict):
        ...
        # ML model parameters
        self.rf_params = config.get('random_forest', {
            'n_estimators': 100,
            'max_depth': 10,
            'min_samples_split': 5,
            'min_samples_leaf': 2,
        ...
        self.gb_params = config.get('gradient_boosting', {
            'n_estimators': 100,
            'max_depth': 6,
            'learning_rate': 0.1,
            'random_state': 42
        ...
        print(f"🤖 ML Signal Confirmation initialized with confidence threshold: {self.confidence_threshold}")
    ...
    def prepare_training_data(self, historical_data: List[Tuple], lookback_days: int = 30) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        for i in range(lookback_days, len(historical_data)):
            ...
            # Engineer features
            features = self.engineer_features([bar[0] for bar in bars], current_indicators)
            if features.size == 0:
                continue
    ...
    def confirm_signal(self, bars: List[Bar], indicators: Dict) -> Tuple[bool, float, Dict]:
        """
        ...
        for name, model in self.models.items():
            ...
            confidences[name] = proba[1] if len(proba) > 1 else proba[0]  # Probability of positive class
        ...
        signal_confirmed = ensemble_prediction > 0.5 and avg_confidence > self.confidence_threshold
