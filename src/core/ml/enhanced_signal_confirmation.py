...
class EnhancedMLSignalConfirmation:
    ...
    def __init__(self, config: Dict):
        ...
        self.model_configs = {
            ...
            'adaboost': config.get('adaboost', {
                'n_estimators': 100,
                'learning_rate': 0.1,
                'random_state': 42
            }),
            'extra_trees': config.get('extra_trees', {
                'n_estimators': 200,
                'max_depth': 15,
                'min_samples_split': 3,
                'random_state': 42
        ...
        print(f"🤖 Enhanced ML Signal Confirmation initialized")
        print(f"   Ensemble Method: {self.ensemble_config['voting_method']}")
        print(f"   Models: {len(self.model_configs)} algorithms")
        print(f"   Feature Selection: {self.feature_config['feature_selection_method']}")
        print(f"   Online Learning: {self.adaptation_config['online_learning']}")
    
    def engineer_advanced_features(self, bars: List[Bar], indicators: Dict) -> np.ndarray:
        """
        ...
        for period in self.feature_config['lookback_periods']:
            if len(closes) >= period:
                ...
                hl_spread = (max(highs[-period:]) - min(lows[-period:])) / np.mean(closes[-period:])
                features.append(hl_spread)
                feature_names.append(f'hl_spread_{period}')
                
                # Price acceleration (second derivative)
                if period >= 10:
                    recent_momentum = (closes[-1] - closes[-period//2]) / closes[-period//2]
                    past_momentum = (closes[-period//2] - closes[-period]) / closes[-period]
                ...
                feature_names.extend([
                    f'momentum_{period}', f'volatility_{period}', f'volume_momentum_{period}',
                    f'hl_spread_{period}', f'acceleration_{period}', f'trend_strength_{period}'
                ])
        ...
        if self.feature_config['use_market_microstructure']:
            ...
            volume_profile = np.percentile(volumes[-20:], [25, 50, 75]) if len(volumes) >= 20 else [0, 0, 0]
            features.extend([
                volume_profile[0] / np.mean(volumes[-20:]) if len(volumes) >= 20 else 0,
                volume_profile[2] / np.mean(volumes[-20:]) if len(volumes) >= 20 else 0
            ])
            feature_names.extend(['volume_q25', 'volume_q75'])
            
            # Price impact
            price_impact = abs(closes[-1] - closes[-2]) / volumes[-1] if volumes[-1] > 0 else 0
        ...
        feature_names.extend(['fast_ma', 'slow_ma', 'trend_ma', 'short_ma', 'medium_ma', 'long_ma'])
        ...
        if 'correlation_features' in indicators:
            ...
            feature_names.extend([f'corr_{i}' for i in range(len(indicators['correlation_features']))])
    ...
    def prepare_enhanced_training_data(self, historical_data: List[Tuple], lookback_days: int = 60) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        for i in range(lookback_days, len(historical_data)):
            ...
            # Engineer features
            features = self.engineer_advanced_features([bar[0] for bar in bars], current_indicators)
            if features.size == 0:
                continue
            
            # Enhanced labeling strategy
            if i + 10 < len(historical_data):
                future_prices = [float(historical_data[j][0].close) for j in range(i, min(i+10, len(historical_data)))]
                current_price = float(bars[-1][0].close)
                
                # Calculate multiple future returns
                returns_5 = (future_prices[4] - current_price) / current_price if len(future_prices) > 4 else 0
                returns_10 = (future_prices[-1] - current_price) / current_price if len(future_prices) > 9 else 0
    ...
    def train_enhanced_models(self, historical_data: List[Tuple], validation_split: float = 0.2):
        """
        ...
        if len(X) < self.config.get('min_training_samples', 100):
            print(f"⚠️ Insufficient training data. Need at least {self.config.get('min_training_samples', 100)} samples.")
            return False
        ...
        X_train_selected, X_val_selected, selected_features = self._select_features(X_train, X_val, y_train)
        ...
        if len(individual_models) > 1:
            ...
            ensemble = VotingClassifier(
                ...
                weights=list(model_weights.values()) if self.ensemble_config['use_weights'] else None
            )
    ...
    def _select_features(self, X_train: np.ndarray, X_val: np.ndarray, y_train: np.ndarray) -> Tuple[np.ndarray, np.ndarray, object]:
            ...
            if hasattr(model, 'feature_importances_'):
                ...
                if hasattr(feature_selector, 'get_support'):
                    ...
                    if hasattr(self, 'feature_names'):
                        selected_names = [self.feature_names[i] for i, selected in enumerate(selected_indices) if selected]
                        importances = model.feature_importances_
                        self.feature_importance[name] = dict(zip(selected_names, importances))
    
    def confirm_signal_enhanced(self, bars: List[Bar], indicators: Dict) -> Tuple[bool, float, Dict]:
        """
        ...
        if 'main' in self.feature_selectors and self.feature_selectors['main'] is not None:
        ...
        ensemble_confidence = ensemble_model.predict_proba(features_scaled)[0, 1] if hasattr(ensemble_model, 'predict_proba') else 0.5
        ...
        for name, model in self.models.items():
            if name != 'ensemble':
                try:
                    ...
                    conf = model.predict_proba(features_scaled)[0, 1] if hasattr(model, 'predict_proba') else 0.5
        ...
        signal_confirmed = (
            ensemble_prediction > 0.5 and 
            adjusted_confidence > confidence_threshold and
        ...
        self._store_adaptation_data(ensemble_prediction, adjusted_confidence, uncertainty, signal_confirmed)
    ...
    def _store_adaptation_data(self, prediction: float, confidence: float, uncertainty: float, confirmed: bool):
        ...
        if self.adaptation_config['online_learning'] and len(self.adaptation_history) >= self.adaptation_config['adaptation_frequency']:
            self._adapt_models()
    ...
    def _adapt_models(self):
        ...
        recent_accuracy = sum(1 for h in recent_history if h['confirmed']) / len(recent_history)
        ...
        if recent_accuracy < self.adaptation_config['performance_threshold']:
            ...
            if recent_accuracy < 0.4:
                self.confidence_config['confidence_threshold'] = min(0.9, self.confidence_config['confidence_threshold'] + 0.05)
            elif recent_accuracy < 0.6:
                self.confidence_config['confidence_threshold'] = min(0.9, self.confidence_config['confidence_threshold'] + 0.02)
            
            print(f"   Adjusted confidence threshold to: {self.confidence_config['confidence_threshold']:.3f}")
    ...
    def load_enhanced_models(self, filepath: str):
        ...
        try:
            # Load models
            for name in ['ensemble', 'random_forest', 'gradient_boosting', 'adaboost', 'extra_trees', 'neural_network', 'svm']:
                model_path = f"{filepath}_{name}.joblib"
