...
from ..data_models.market import MarketData, Bar, Feature
from ..data_models.trading import Portfolio, Position, Order, OrderSide, OrderType
...
class TradingEngine:
    ...
    def generate_signals(self, symbol: str) -> Dict:
        ...
        for strategy_name, strategy in self.strategies.items():
            ...
            try:
                ...
                logger.logger.error(f"Error generating signals for {strategy_name}: {e}")
    ...
    def execute_signals(self, signals: Dict) -> List[Order]:
        ...
        for strategy_name, strategy_signals in signals.items():
            ...
            for symbol, entry_details in entry_signals.items():
                try:
                    order = self._create_entry_order(symbol, entry_details, strategy_name)
                    if order:
                        orders.append(order)
    ...
    def _create_entry_order(self, symbol: str, entry_details: Dict, strategy_name: str) -> Optional[Order]:
        ...
        logger.logger.info(f"Created {direction} entry order for {symbol}: {position_size}")
        return order
    
    def _create_exit_order(self, symbol: str, exit_details: Dict, strategy_name: str) -> Optional[Order]:
        ...
        for position in self.portfolio.positions:
            if position.symbol == market_data.symbol:
                ...
                if position.is_long:
                    ...
                    position.unrealized_pnl = (position.average_cost - current_price) * abs(position.quantity)
    ...
    def _recalculate_portfolio_metrics(self) -> None:
        ...
        # Calculate exposure
        total_position_value = sum(abs(float(pos.market_value)) for pos in self.portfolio.positions)
        ...
        if self.portfolio.peak_equity > 0:
            drawdown = (self.portfolio.peak_equity - self.portfolio.equity) / self.portfolio.peak_equity
    ...
    def get_performance_summary(self) -> Dict:
        ...
        summary = {
            "portfolio": {
                ...
                "exposure": float(self.portfolio.exposure_gross) if self.portfolio else 0,
                "drawdown": float(self.portfolio.drawdown) if self.portfolio else 0
            },
            "risk_metrics": self.risk_manager.get_risk_metrics(self.portfolio) if self.portfolio else {},
            "strategies": {}
        }
