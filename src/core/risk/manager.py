...
from ..data_models.risk import RiskEvent, RiskEventType, RiskEventSeverity, CircuitBreaker
...
class RiskManager:
    ...
    def check_position_limits(self, position: Position, portfolio: Portfolio) -> Tuple[bool, Optional[str]]:
        ...
        max_position_size = self.config.get("position", {}).get("max_size_per_symbol", 0.1)
        ...
        if portfolio_value > 0:
            ...
            if position_ratio > max_position_size:
                return False, f"Position size {position_ratio:.2%} exceeds limit {max_position_size:.2%}"
        
        return True, None
    
    def check_portfolio_limits(self, portfolio: Portfolio) -> Tuple[bool, Optional[str]]:
        ...
        max_gross_exposure = self.config.get("portfolio", {}).get("max_gross_exposure", 0.5)
        if portfolio.equity > 0:
            gross_exposure_ratio = float(portfolio.exposure_gross) / float(portfolio.equity)
            if gross_exposure_ratio > max_gross_exposure:
                return False, f"Gross exposure {gross_exposure_ratio:.2%} exceeds limit {max_gross_exposure:.2%}"
        ...
        if portfolio.equity > 0:
            ...
            if cash_ratio < min_cash_reserve:
                return False, f"Cash reserve {cash_ratio:.2%} below minimum {min_cash_reserve:.2%}"
        
        return True, None
    
    def update_circuit_breakers(self, portfolio: Portfolio, market_data: Optional[MarketData] = None) -> List[str]:
        ...
        if portfolio.equity > 0:
            ...
            self.circuit_breakers["daily_loss"].current_value = Decimal(str(daily_loss_ratio))
        ...
        if portfolio.equity > 0:
            ...
            self.circuit_breakers["drawdown"].current_value = Decimal(str(drawdown_ratio))
        ...
        if market_data and hasattr(market_data, 'features'):
            ...
            if volatility is not None:
                self.circuit_breakers["volatility"].current_value = Decimal(str(volatility))
        ...
        if market_data and market_data.spread_relative is not None:
            ...
            self.circuit_breakers["spread_stress"].current_value = Decimal(str(spread_ratio))
    ...
    def _trigger_circuit_breaker(self, breaker_name: str, value: float) -> None:
        ...
        if not breaker.triggered:
            ...
            breaker.triggered_timestamp = datetime.now(timezone.utc)
    ...
    def get_risk_metrics(self, portfolio: Portfolio) -> Dict:
        ...
        if portfolio.equity > 0:
            ...
            total_position_value = sum(abs(float(pos.market_value)) for pos in portfolio.positions)
            metrics["position_concentration"] = total_position_value / float(portfolio.equity)
            ...
            # Exposure ratios
            metrics["gross_exposure_ratio"] = float(portfolio.exposure_gross) / float(portfolio.equity)
            metrics["net_exposure_ratio"] = float(portfolio.exposure_net) / float(portfolio.equity)
    ...
    def can_trade(self, portfolio: Portfolio) -> Tuple[bool, Optional[str]]:
        ...
        active_breakers = [name for name, breaker in self.circuit_breakers.items() if breaker.is_active]
        
        if active_breakers:
            return False, f"Trading blocked by active circuit breakers: {', '.join(active_breakers)}"
    ...
    def get_position_size_limit(self, symbol: str, portfolio: Portfolio) -> Decimal:
        ...
        """
        max_position_ratio = self.config.get("position", {}).get("max_size_per_symbol", 0.1)
