...
from ..data_models.trading import Fill, Portfolio, Position
...
class BacktestEngine:
    ...
    async def run_backtest(
        ...
        try:
            ...
            logger.info(
                f"Backtest completed. Final portfolio value: {self.portfolio.equity}"
            )
            return results
    ...
    async def _load_historical_data(
        ...
        if not any(
            adapter.is_connected() for adapter in self.data_manager.adapters.values()
        ):
    ...
    async def _run_simulation(self):
        ...
        if not sorted_timestamps:
            ...
            self._record_portfolio_state(datetime.now(UTC))
            return
    ...
    def _create_market_data_snapshot(self, timestamp: datetime) -> MarketData:
        ...
        for symbol, symbol_bars in self.historical_data.items():
            ...
            if closest_bar:
                ...
                bars_up_to_timestamp = [
                    bar for bar in symbol_bars if bar.timestamp <= timestamp
                ]
                bars[symbol] = bars_up_to_timestamp

                # Calculate features for this bar
                symbol_features = self._calculate_features(
                    symbol, symbol_bars, closest_bar
                )
                if symbol_features:
    ...
    def _generate_signals(self, market_data: MarketData) -> list[dict[str, Any]]:
        ...
        try:
            print(
                f"🔍 Generating signals for market data with {len(market_data.bars)} symbols"
            )
            ...
            for symbol, signal_data in strategy_signals.get(
                "entry_signals", {}
            ).items():
                signals.append(
                    {
    ...
    async def _execute_buy_order(self, signal: dict[str, Any], timestamp: datetime):
        ...
        if position_size > self.portfolio.cash:
            logger.warning(
                f"Insufficient cash for {symbol} buy order. Need ${position_size:,.2f}, have ${self.portfolio.cash:,.2f}"
            )
            return
        ...
        if existing_position:
            ...
            total_cost = (
                existing_position.average_cost * existing_position.quantity
                + position_size
            )
        ...
        self.total_trades += 1
        logger.info(
            f"Executed BUY order: {quantity} {symbol} @ ${price:,.2f} for ${position_size:,.2f}"
        )
    ...
    async def _execute_sell_order(self, signal: dict[str, Any], timestamp: datetime):
        ...
        if abs(position.quantity) < quantity:
            logger.warning(
                f"Insufficient quantity for {symbol} sell order. Have {abs(position.quantity)}, need {quantity}"
            )
            return
        ...
        logger.info(
            f"Executed SELL order: {quantity} {symbol} @ ${price:,.2f}, P&L: ${pnl:+,.2f}"
        )
            ...
            for position in self.portfolio.positions:
                if position.symbol == symbol:
                    ...
                    if position.quantity > 0:  # Long position
                        ...
                        unrealized_pnl = position.market_value - (
        ...
        self.portfolio_snapshots.append(
            {
                ...
                if hasattr(self.portfolio, "copy")
                else self.portfolio,
            }
        )
    ...
    def _record_hourly_pnl(self, timestamp: datetime):
        ...
        hourly_pnl_pct = (
            (hourly_pnl / previous_equity * 100) if previous_equity > 0 else 0
        )
        ...
        for trade in self.trades:
            if (
                trade["timestamp"].hour == timestamp.hour
                and trade["timestamp"].date() == timestamp.date()
            ):
        ...
        self.hourly_pnl.append(
            {
                ...
                "cash": float(self.portfolio.cash),
                "exposure": float(
                    sum(abs(pos.market_value) for pos in self.portfolio.positions)
                ),
            }
        )
    ...
    def _calculate_results(self) -> BacktestResult:
        ...
        if abs(calculated_equity - self.portfolio.equity) > Decimal("0.01"):
            logger.warning(
                f"Portfolio equity discrepancy detected: stored={self.portfolio.equity}, calculated={calculated_equity}"
            )
        ...
        if total_return > 10.0:  # More than 1000%
            logger.error(
                f"Unrealistic total return detected: {total_return:.2%}. This indicates a calculation error."
            )
        ...
        if self.start_time and self.end_date:
            ...
            if duration_days > 0:
                ...
                if abs(annualized_return) > 100.0:  # More than 10000% annualized
                    logger.error(
                        f"Unrealistic annualized return detected: {annualized_return:.2%}. This indicates a calculation error."
                    )
                    annualized_return = 0.0
            else:
                annualized_return = 0.0
        ...
        total_loss = abs(
            sum(trade.get("pnl", 0) for trade in self.trades if trade.get("pnl", 0) < 0)
        )
        ...
        logger.info(
            f"Results calculated: Initial=${initial_equity:,.2f}, Final=${final_equity:,.2f}, Return={total_return:.2%}"
        )
