...
class DataManager:
    ...
    async def get_historical_data(
        ...
        if adapter_name and adapter_name in self.adapters:
            ...
            if adapter.is_enabled():
                return await adapter.get_historical_data(symbol, timeframe, limit, since)
        
        # Try all adapters in order of preference
        for name, adapter in self.adapters.items():
            if adapter.is_enabled() and adapter.is_connected():
                try:
                    data = await adapter.get_historical_data(symbol, timeframe, limit, since)
                    if data:
                        # Cache the data
    ...
    async def get_current_price(self, symbol: str, adapter_name: Optional[str] = None) -> Optional[Decimal]:
