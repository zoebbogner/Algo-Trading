...
def show_comprehensive_results():
    ...
    for symbol, data in CRYPTO_SYMBOLS.items():
        if symbol != "BTC/USDT":
            bitcoin_correlations.append(
                (symbol, data.bitcoin_correlation, data.category.value)
            )
    ...
    for i, symbol in enumerate(diversification_symbols[:10]):
        if symbol in CRYPTO_SYMBOLS:
            ...
            print(
                f"   {i+1:2d}. {symbol:12} | {data.bitcoin_correlation:5.2f} | {data.category.value}"
            )
    ...
    regimes = [
        (
            "🐂 BULL MARKET",
            "Bitcoin > +10% daily",
            "Correlations increase, most coins follow Bitcoin",
            "High correlation pairs become more profitable",
        ),
        (
            "🐻 BEAR MARKET",
            "Bitcoin < -10% daily",
            "Correlations increase, panic selling affects all",
            "Diversification becomes crucial",
        ),
        (
            "➡️ SIDEWAYS",
            "Bitcoin ±5% daily",
            "Correlations decrease, individual coin fundamentals matter",
            "Focus on coin-specific analysis",
        ),
    ]
    ...
    sorted_categories = sorted(
        category_correlations.items(), key=lambda x: abs(x[1]), reverse=True
    )
    ...
    print(
        f"High Correlation (≥0.7): {len(high_corr)} ({len(high_corr)/len(all_symbols)*100:.1f}%)"
    )
    print(
        f"Medium Correlation (0.4-0.7): {len(medium_corr)} ({len(medium_corr)/len(all_symbols)*100:.1f}%)"
    )
    print(
        f"Low Correlation (≤0.4): {len(low_corr)} ({len(low_corr)/len(all_symbols)*100:.1f}%)"
    )
    print()
