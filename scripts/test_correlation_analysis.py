...
from src.config.crypto_symbols import get_all_symbols, get_symbols_by_category, get_diversification_recommendations
...
async def test_correlation_analysis():
    ...
    try:
        ...
        if hasattr(correlation_analyzer, 'correlation_matrix') and not correlation_analyzer.correlation_matrix.empty:
    ...
    try:
        ...
        bitcoin_impacts = await correlation_analyzer.analyze_bitcoin_impact(target_symbols)
        print(f"✅ Bitcoin impact analysis completed: {len(bitcoin_impacts)} symbols analyzed")
        ...
        if bitcoin_impacts:
            ...
            for i, (symbol, correlation, beta) in enumerate(bitcoin_dependent[:5]):
                print(f"   {i+1}. {symbol}: Correlation={correlation:.3f}, Beta={beta:.3f}")
    ...
    try:
        ...
        for category in categories:
            ...
            print(f"📊 {category.value.upper()} category ({len(category_symbols)} symbols):")
    ...
    try:
        ...
        print()
        print("💡 These symbols have low Bitcoin correlation and can help diversify your portfolio")
